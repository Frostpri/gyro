...
def test(test_ids_P, test_ids_V, all=False, stride=WINDOW_SIZE[0], batch_size=BATCH_SIZE, window_size=WINDOW_SIZE):
    ...
    test_imagess = (1 / 255 * np.asarray(io.imread(DATA_FOLDER_P.format(id))[:, :, :3], dtype='float32') for id in test_ids_P)
    ...
    with torch.no_grad():
        for imgs, imgt, gt, gt_e in tqdm(zip(test_imagess, test_images, test_labels, eroded_labels), total=len(test_ids_V), leave=False):
            ...
            for i, coords in enumerate(
                ...
                # Build the tensor
                coordss = ((random.choice(range(5000)), random.choice(range(5000)), 256, 256),)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch_idx, (images, labels, images_t, labels_t) in enumerate(train_loader):
            ...
            if iter % 100 == 0:
                ...
                start_time = time.time()
                if MIoU > MIoU_best:
                    torch.save(net.state_dict(), './Train_Model/DATrans_PRGB2V_epoch{}_{}'.format(epoch, MIoU))
                    MIoU_best = MIoU
    print("Train Done!!")
...
#     # io.imsave('./Test_Vision/PRGB2V_tile_{}.png'.format(id_), img)
