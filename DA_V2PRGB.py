...
def test(test_ids_P, test_ids_V, all=False, stride=WINDOW_SIZE[0], batch_size=BATCH_SIZE, window_size=WINDOW_SIZE):
    ...
    test_imagess = (1 / 255 * np.asarray(io.imread(DATA_FOLDER_P.format(id)), dtype='float32') for id in test_ids_P)
    test_images = (1 / 255 * np.asarray(io.imread(DATA_FOLDER_V.format(id))[:, :, :3], dtype='float32') for id in test_ids_V)
    ...
    with torch.no_grad():
        for imgs, imgt, gt, gt_e in tqdm(zip(test_imagess, test_images, test_labels, eroded_labels), total=len(test_ids_V), leave=False):
            ...
            for i, coords in enumerate(
                ...
                # Build the tensor
                coordss = ((random.choice(range(1500)), random.choice(range(1500)), 256, 256),)
                ...
                # Do the inference
                _, _, _, pred_target2, _, _, _, _ = net(image_patchess, image_patches)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch_idx, (images, labels, images_t, labels_t) in enumerate(train_loader):
            ...
            if iter % 100 == 0:
                ...
                start_time = time.time()
                if MIoU > MIoU_best:
                    torch.save(net.state_dict(), './Train_Model/DATrans_V2PRGB_256_epoch{}_{}'.format(epoch, MIoU))
                    MIoU_best = MIoU
    print("Train Done!!")
...
#     io.imsave('./Test_Vision/V2P_tile_{}.png'.format(id_), img)